...
node {
    ...
    withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
        stage('Deploye Code') {
            ...
            if (isUnix()) {
				rmsg = sh returnStdout: true, script: "${toolbelt} force:source:deploy --manifest manifest/destructiveChangesPre.xml --predestructivechanges -u thiago.xaviercosta@portoseguro.com.br.bu"
			}else{
			   rmsg = bat returnStdout: true, script: "\"${toolbelt}\" force:source:deploy --manifest manifest/destructiveChangesPre.xml --predestructivechanges -u thiago.xaviercosta@portoseguro.com.br.bu"
			}
